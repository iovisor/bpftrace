...
////
Custom prefix to easily link to a language section
////
:idprefix: language-
...
!
!
This prefix reset must be at the end of the language section, else cross references break
!
!
////
...
Equal to <<functions-ksym>> but resolves user space symbols
...
| r
| buffer
| Hex-formatted string to print arbitrary binary content returned by the buf (<<functions-buf>>) function.
